...
export default function Students() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Referral banner — solo stats, abre el mismo modal de invitar */}
      {referrals.length > 0 && (
        <Card
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-gray-900 text-sm">Referidos</p>
              <p className="text-xs text-gray-500">
                Invitaste a {referrals.length} persona{referrals.length !== 1 ? 's' : ''} —{' '}
                {referrals.filter(r => r.profiles?.role === 'trainer').length} entrenador{referrals.filter(r => r.profiles?.role === 'trainer').length !== 1 ? 'es' : ''},{' '}
                {referrals.filter(r => r.profiles?.role === 'student').length} alumno{referrals.filter(r => r.profiles?.role === 'student').length !== 1 ? 's' : ''}
              </p>
            </div>
